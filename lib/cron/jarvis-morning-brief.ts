...
const DATED_REMINDERS: Record<string, string[]> = {
  ...
  "2026-07-27": [
    "MONDAY — DIAL DAY 1 of 2, YOUR schedule: dial block 1 9–11am (you're in town — every row is tap-to-call from the phone), home for lunch → run the migration 12–1, dial block 2 from 2pm. One job: get 15-minute demos BOOKED. Warm dialling — every name already had your email or DM.",
    "⓪ 8:45, TEN MINUTES: read this brief, answer any replies FIRST (warmer than any dial), demo page + booking link one tap away. Know the founding offer cold (first 10 customers, rates rise after — that's your urgency). Rule: 20 dials before you judge the day.",
    "① LUNCHTIME (12–1): migrations are already RUN (done Sunday — all 7 tables confirmed). Just check automateiq.ie/tradeos loads clean after a hard refresh (if it still errors, send Claude the digest code from the error page). Then 5 min in /finance: set your bank balance, scan one supplier invoice, and try the new Assistant tab. Log any morning calls you missed while you're there.",
    "② THE LIST (both blocks): Prospects → tick 'Has phone' → sort by Lead score. Work Status='Contacted' first, then 'Follow-up sent'. Each prospect page opens with 'Outreach so far' — glance it, then open with 'I emailed you on X about Y, wanted to put a voice to it.'",
    "③ THE ASK on every call: a 15-minute demo THIS WEEK — you're booking the meeting, not pitching on the phone. Pin it to a date. LOG EVERY CALL with the one-tap 'Log call' button so the follow-up auto-schedules and nothing leaks. Voicemail = a touch: 15 seconds, log it, next.",
    "④ BLOCK 2 (2pm): keep dialling — stretch to 3:30–4 if the morning went well. Last 10 min: queue ~20 best drafts for Tuesday's 8am run and glance 'Awaiting outcome' on Meetings.",
    "TARGET: ~25–30 dials across the two blocks, every one logged, 2–3 demos booked. Tomorrow you dial again AND start closing the earliest bookings.",
  ],
  "2026-07-28": [
