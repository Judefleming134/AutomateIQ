...
export async function researchProspect(_prev: Result, formData: FormData): Promise<Result> {
  ...
  // The research/persist/park logic is SHARED with the overnight research
  // worker (lib/growth/research-runner.ts) — one implementation, so the
  // unattended path can never drift from what this button does.
  let result;
  try {
  ...
  const persisted = await persistResearchResult(
    ...
    member.id,
    (score) =>
      `Company research completed by ${member.name} (${
