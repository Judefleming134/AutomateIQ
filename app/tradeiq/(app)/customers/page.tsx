      ...
      <section className="panel panel-block">
        {rows.length === 0 ? (
          <p className="empty-state">
            No customers yet. <Link href="/tradeiq/new">Create a quote →</Link> and the customer is saved here automatically.
          </p>
        ) : (
                ...
                {rows.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td style={{ textAlign: "right" }}>
                      <Link href="/tradeiq/new" className="btn btn-ghost btn-sm">Quote →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
