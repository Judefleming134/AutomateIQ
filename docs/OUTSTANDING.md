...
Last reviewed: 2026-08-02 daytime (**THE NEEDS-JUDE LIST IS NOW ONE ITEM.**
J2 confirmed by Jude (Resend domain verified); J5 and J6 closed on his "rest is
okay for now"; J1 was removed by building the free Google self-check; J3 was
decided and shipped. What's left is a single SQL paste —
`supabase/bundles/full_schema.sql`, every migration 0001-0042 in one
transaction, rewritten so it is safe to run against a database in ANY state.
He does not have to know which migrations he already ran. Validated on scratch
Postgres 16 four ways: a fresh database builds the whole schema; the identical
paste run twice changes nothing; a database already at 0034 holding 50
businesses / 2,000 prospects / 500 messages / 300 bookings / 400 quotes gains
only what was missing and loses no row; and an injected failure part-way
through leaves zero tables behind. The fresh and the part-migrated routes
finish byte-identical. Generated by `scripts/build-schema-bundle.mjs`, guarded
by `lib/db/schema-bundle.test.ts` so it cannot go stale, and the migration
files themselves are untouched — they are the record of what was applied.
Earlier today: the Google checker stopped needing a paid API key (J1), the GBP
scorer got its first 40 tests (F7), booking lead time 24h->12h so the call
script's "tomorrow morning" is bookable (J3), "Log call" stopped reporting a
follow-up it never scheduled, and the Message Studio stopped opening on
channels a prospect can't be reached on.) Earlier: 2026-08-03 nightly (SSRF
closed via safeFetch per-hop re-validation; prospects page 42,011 rows in 55
requests -> 347 in 3.)
...
**One item.** J1 (Google key) was removed by building the free self-check; J3
was decided and shipped; J2, J5 and J6 were confirmed/closed by Jude on
2026-08-02. What remains is a single SQL paste.

| # | Item | Why it's blocking | Effort |
|---|---|---|---|
| **JX** | **Run ONE file: `supabase/bundles/full_schema.sql`** | Every migration in the repo (0001–0042, 34 files), in order, in a single transaction, rewritten to be safe against a database in ANY state. **You do not need to know which migrations you have already run** — run it and the database ends up correct either way; run it twice and the second run changes nothing. Supabase → SQL Editor → New query → paste → Run. It never drops a table, never drops a column, never deletes a row; the only things replaced are functions, views, policies and triggers. Generated by `scripts/build-schema-bundle.mjs` — the migration files themselves are untouched. Validated 2026-08-02 on scratch Postgres 16 four ways: (1) a fresh database builds the whole schema, (2) the identical paste run a second time changes nothing, (3) a database already migrated to 0034 holding 50 businesses / 2,000 prospects / 500 messages / 300 bookings / 400 quotes gains only what was missing and loses **no row**, (4) an injected failure part-way through leaves **zero** tables behind. Routes (1) and (3) finish with a byte-identical column signature and the same 40 policies, 8 triggers and 48 RLS-protected tables. `pending_0035_to_0042.sql` is kept alongside it and still works, for anyone who wants only the outstanding eight. | **3 min, once** |
