...
| J7 | **Paste three migrations into the Supabase SQL editor, in order: `0031_send_target_50.sql`, `0032_agent_runs.sql`, `0033_permitiq.sql`** | Nothing in this repo applies migrations — no CI step, no Supabase CLI in any workflow — so a migration file is inert until Jude pastes it. All three are validated on scratch PG16 and idempotent; re-running any of them is a no-op. **0031** sets the daily send target to 50 and is safe whether or not 0030 went in; until it runs, saving at `/growth/settings` fails if 0030 is also unapplied. **0032** creates `agent_runs` (the agent log); until it runs the assistant behaves exactly as today and the insert is swallowed with a log line. **0033** creates the PermitIQ schema, the private `permits` bucket, the product row and the Irish requirements catalog; until it runs, PermitIQ shows as Coming soon and nothing else changes. | 6 min |
