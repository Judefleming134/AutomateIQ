...
| J7 | **Paste two migrations into the Supabase SQL editor: `0031_send_target_50.sql` then `0032_agent_runs.sql`** | Nothing in this repo applies migrations — no CI step, no Supabase CLI in any workflow — so a migration file is just a file until Jude pastes it. **0031** sets the daily send target to the 50 he asked for and is safe whether or not 0030 ever went in (validated on scratch PG16, both states, idempotent); until it runs, **saving anything at `/growth/settings` will fail** if 0030 is also unapplied, because that write names `daily_send_target`. **0032** creates `agent_runs`, the Agent Framework v2 run log (validated on scratch PG16: idempotent, CHECK constraints, tenant isolation, suspended-tenant cutoff, no tenant writes, cascade). Until 0032 runs the assistant works exactly as before — the log insert fails, is swallowed, and says so in the server logs. | 4 min |
